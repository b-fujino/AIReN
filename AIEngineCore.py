...
class InterviewerEngine:
    ...
    def __init__(self):
        ...
        self.output_dir = f"{p_dir}/Run_{time.strftime('%Y%m%d_%H%M%S')}" # 出力ディレクトリ
    ...
    def _Summarize(self, Question, Report):
        ...
        smry = Agent_chat(
            ...
            temperature=0.0,
            stream=False,
            num_predict=2048, #要約の出力トークンの最大値．512はあくまで目安。要約が長くなりそうなときには増やす必要があるかも。
            Debug=bDEBUG
        )
        print(smry)
    ...
    def run(self, Question=None, Report=None, Stream=False):
        ...
        if self.instructions: # もしself.instructionsが空でなかったら，Supervisorの指示に基づく追加質問を実施する．
            ...
            if result.go_next: #str(result["go_next"]).lower() == "true":  # If the result is clear, break the loop
                ...
                同一のものがないときには，self.instructionsに追加する．
                '''
...
def AIReNTest(bSTREAM=False, turn_num=0, idx=0):
    ...
    print(f"Final Summary:\n{final_summary.model_dump_json(indent=2, ensure_ascii=False)}")
    
    with open(f"{p_dir}/SCENARIO_{idx}_GPT_AIReN_turn_{turn_num}_{time.strftime('%Y%m%d_%H%M%S')}_debug.json", "w", encoding="utf-8") as f:
        f.write(final_summary.model_dump_json(indent=2, ensure_ascii=False))
